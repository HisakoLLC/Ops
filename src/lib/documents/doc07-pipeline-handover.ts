  ...
  return new Document({
    sections: [
      buildSection([
        ...
        p("Business Impact:", true),
        p(data.business_impact || "N/A"),
        
        p(""),
        tag("TRIGGER"),
        p(data.trigger_description || "N/A"),
        
        h2("2. Workflow Steps"),
        ...(data.steps || data.pipeline_steps || []).map((step: any) => 
          p(`${step.step_number || step.step_label || ""}. ${step.description || step.step_description || ""}`)
        ),
        ...
        ...(Array.isArray(data.common_issues) && data.common_issues.length > 0
          ? data.common_issues.map((issue: any) => 
              bullet(`${issue.symptom || ""} -> Cause: ${issue.cause || ""} -> Fix: ${issue.fix || ""}`)
            )
