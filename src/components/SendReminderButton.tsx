    ...
    try {
      const res = await fetch('/api/email/send', {
        ...
        body: JSON.stringify({
          ...
          templateData: { message: message || undefined, invoiceRef, amount, dueDate, clientName, isReminder: true, invoiceType: 'adhoc' },
        }),
      })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
      toast.success(`Reminder for ${invoiceRef} sent to ${to}`)
      setOpen(false)
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Send Reminder {invoiceRef}</DialogTitle>
          </DialogHeader>
